...
class TashiDroneController:
    ...
    def _confirm_delivery(self):
        ...
        delivered_early = time.time() < self.state_machine.current_delivery.bid_deadline
        ...
        confirmation = {
            "type": MessageType.DELIVERY_CONFIRMATION.value,
        ...
        print(f"[{self.name}] Delivery confirmed for {request_id}")
    ...
    def run(self):
        ...
        while self.robot.step(self.timestep) != -1:
            ...
            # Set home waypoint from live pose once sensors are stable.
            if not self.flight_ready and count > 10:
                ...
                print(f"[{self.name}] Home waypoint set to {self.home_location}")

            # Legacy handshake for compatibility.
            ...
            if current_state == DeliveryState.NAVIGATING_PICKUP and self.state_machine.current_delivery:
                ...
                self.target_waypoint = {
                    "x": x_pos,
                    "y": y_pos,
                    "z": self.home_location.get("z", 0.2),
                }
                self.target_altitude = max(self.home_location.get("z", 0.2), 0.2)
                self._apply_flight_control(
                    roll,
                    pitch,
                    yaw,
